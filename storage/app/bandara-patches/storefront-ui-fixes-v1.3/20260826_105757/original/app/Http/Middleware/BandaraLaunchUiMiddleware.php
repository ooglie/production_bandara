...
final class BandaraLaunchUiMiddleware
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $response->headers->set('X-Bandara-Launch-UI', '1');
    ...
    private function replaceLaunchCopy(string $html): string
    {
        ...
        $html = str_replace('"recipe_limit": 3', '"recipe_limit": 1', $html);
    ...
    private function injectStateMeta(string $html, bool $newsletterSubscribed): string
    {
        ...
        $meta = sprintf(
            ...
            '<meta name="bandara-launch-ui" content="1">'."\n",
    ...
    private function injectRewardBanner(string $html, string $fragment): string
    {
        ...
        if (preg_match('~</header>~i', $html) === 1) {
            return preg_replace('~</header>~i', '</header>'.$fragment, $html, 1) ?? $html;
        }
    ...
    {
        return <<<'HTML'
<div class="bandara-launch-reward" data-bandara-reward-banner role="region" aria-label="Bandara welcome credit">
