...
<a href="{{ route('kitchen.chefs.show', $chef) }}"
   class="group flex h-full flex-col overflow-hidden rounded-xl border border-slate-200/80 bg-white transition duration-300 hover:-translate-y-1 hover:border-slate-300 hover:shadow-lg dark:border-slate-800 dark:bg-slate-950 dark:hover:border-slate-700">
    <div class="aspect-[4/5] overflow-hidden bg-slate-100 dark:bg-slate-900">
        @if ($portraitUrl)
            <img src="{{ $portraitUrl }}"
                 ...
                 loading="lazy">
        @else
            <div class="flex h-full items-center justify-center text-3xl font-light tracking-[0.18em] text-slate-400 dark:text-slate-600">
    ...
    <div class="flex flex-1 flex-col gap-2 p-4 sm:p-5">
        <div>
            <h3 class="text-lg font-light tracking-tight text-slate-950 dark:text-white">
                {{ $chef->display_name }}
            </h3>
            <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">
                {{ $chef->professional_title }}
                @if ($organisation)
                    <span aria-hidden="true"> · </span>{{ $organisation }}
                @endif
            </p>
            <p class="mt-1 text-xs uppercase tracking-[0.14em] text-slate-500 dark:text-slate-500">
                {{ collect([$chef->city, $chef->country])->filter()->implode(' · ') }}
            </p>
        </div>

        @if ($chef->specialtyList() !== [])
            <p class="line-clamp-2 text-sm leading-6 text-slate-600 dark:text-slate-400">
                {{ collect($chef->specialtyList())->take(3)->implode(' · ') }}
            </p>
        @endif

        <span class="mt-auto inline-flex items-center gap-2 pt-3 text-sm font-medium text-slate-900 dark:text-slate-100">
            Meet the chef
