<?php $__env->startSection('title', 'Help & FAQs'); ?>
    ...
    <div class="max-w-6xl mx-auto px-4 py-6 space-y-6">
        <div class="min-h-screen bg-stone-50 text-slate-800 dark:bg-slate-950 dark:text-slate-200" data-faq-root>
            ...
            <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
                <nav aria-label="FAQ categories" class="flex gap-2 overflow-x-auto pb-3 lg:flex-wrap">
                    <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <a href="#<?php echo e($category['slug']); ?>" class="shrink-0 rounded-full border border-slate-300 bg-white px-4 py-2 text-xs text-slate-600 transition hover:border-slate-500 hover:text-slate-950 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:border-slate-500 dark:hover:text-white">
            ...
            <div class="mt-10 space-y-14">
                <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <section id="<?php echo e($category['slug']); ?>" class="scroll-mt-28" data-faq-section>
                        <div class="grid gap-7 lg:grid-cols-[0.32fr_0.68fr] lg:gap-12">
                            <div>
                                <p class="text-xs font-normal uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400"><?php echo e(str_pad((string) $loop->iteration, 2, '0', STR_PAD_LEFT)); ?></p>
                                <h2 class="mt-2 text-2xl font-light tracking-tight text-slate-950 dark:text-white"><?php echo e($category['label']); ?></h2>
                                <p class="mt-3 max-w-sm text-sm font-light leading-6 text-slate-500 dark:text-slate-400"><?php echo e($category['description']); ?></p>
                            </div>
                            <div class="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900/70">
                                <?php $__currentLoopData = $category['items']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <details class="group border-b border-slate-200 last:border-b-0 dark:border-slate-800" data-faq-item data-search="<?php echo e(\Illuminate\Support\Str::lower(strip_tags($category['label'].' '.$item['question'].' '.$item['answer']))); ?>">
            ...
            <div class="mt-16 grid gap-5 lg:grid-cols-[1fr_0.52fr]">
                <?php echo $__env->make('pages.partials.contact-card', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                <aside class="rounded-xl border border-slate-200 bg-slate-900 p-6 text-white dark:border-slate-700 dark:bg-slate-100 dark:text-slate-950">
                    ...
                    <div class="mt-6 flex flex-col gap-3 text-sm">
                        <a href="<?php echo e(route('content.terms')); ?>" class="inline-flex items-center justify-between rounded-lg border border-white/20 px-4 py-3 hover:bg-white/10 dark:border-slate-300 dark:hover:bg-slate-200">Terms & Policies <span aria-hidden="true">→</span></a>
                        <a href="<?php echo e(route('content.privacy')); ?>" class="inline-flex items-center justify-between rounded-lg border border-white/20 px-4 py-3 hover:bg-white/10 dark:border-slate-300 dark:hover:bg-slate-200">Privacy Policy <span aria-hidden="true">→</span></a>
                    </div>
                </aside>
            </div>
        </div>
...
<?php echo $__env->make('layouts.customer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/ooglie/Website/ChatGPT/PRODUCTIONFrozen/BandaraFrozen/resources/views/pages/help.blade.php ENDPATH**/ ?>