...
<div class="max-w-6xl mx-auto px-4 py-6 space-y-4 text-xs">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <a href="<?php echo e(route('admin.inventory.packs.index')); ?>" class="rounded border border-gray-300 px-3 py-2 text-xs hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800">Back</a>
    </div>
    ...
    <form method="POST" action="<?php echo e(route('admin.inventory.packs.store')); ?>" class="space-y-5 rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-950" data-transform-stock-form data-output-options-url="<?php echo e(route('admin.inventory.packs.output-options')); ?>">
        <?php echo csrf_field(); ?>
        ...
        <section class="space-y-4">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                <div class="md:col-span-2">
                    ...
                    <select id="source_inventory_lot_id" name="source_inventory_lot_id" required class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-[13px] dark:border-gray-700 dark:bg-gray-900">
                        ...
                        <?php $__currentLoopData = $lots; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lot): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($lot->id); ?>"
                                    data-product-id="<?php echo e($lot->product_id); ?>"
                                    data-variant-id="<?php echo e($lot->product_variant_id); ?>"
                                    data-available-qty="<?php echo e((float) ($lot->available_quantity ?? 0)); ?>"
                                    data-available-weight="<?php echo e((float) ($lot->available_weight_kg ?? 0)); ?>"
                                    data-available-pieces="<?php echo e((float) ($lot->repack_available_piece_count ?? $lot->available_piece_count ?? 0)); ?>"
                                    data-available-full-packs="<?php echo e((int) ($lot->repack_available_pack_count ?? $lot->available_pack_count ?? 0)); ?>"
                                    data-source-pieces-per-unit="<?php echo e($sourcePiecesPerUnitForLot($lot)); ?>"
                                    data-source-unit-weight="<?php echo e((float) ($lot->unit_weight_kg ?? ($lot->productVariant?->product_weight ?? $lot->product?->product_weight ?? 0))); ?>"
                                    data-batch="<?php echo e($lot->batch_code); ?>"
                                    data-expiry="<?php echo e(optional($lot->expiry_date)->format('Y-m-d')); ?>"
                                    data-mode="<?php echo e($lot->inward_mode); ?>"
                                    <?php if((string) $selectedLotId === (string) $lot->id): echo 'selected'; endif; ?>>
                                <?php echo e($lot->product?->name ?? 'Product #' . $lot->product_id); ?>

                                <?php if($lot->productVariant): ?>
                                    — <?php echo e($lot->productVariant->name ?: $lot->productVariant->sku); ?>

                                <?php endif; ?>
                                · Lot #<?php echo e($lot->id); ?> · <?php echo e($lotSummary($lot)); ?>

                                <?php if($lot->batch_code): ?> · Batch <?php echo e($lot->batch_code); ?> <?php endif; ?>
                            </option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                ...
                <div id="source_pieces_wrap">
                    ...
                    <input id="source_pieces_per_unit" name="source_pieces_per_unit" type="number" min="0.001" step="0.001" value="<?php echo e(old('source_pieces_per_unit', 1)); ?>" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-[13px] dark:border-gray-700 dark:bg-gray-900">
        ...
        <section class="space-y-4 border-t border-gray-200 pt-5 dark:border-gray-800">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                <div>
                    ...
                    <input name="packed_date" type="date" value="<?php echo e(old('packed_date', now()->format('Y-m-d'))); ?>" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-[13px] dark:border-gray-700 dark:bg-gray-900">
                </div>
                <div>
                    <label class="mb-1 block text-[12px] font-medium text-gray-700 dark:text-gray-300">Expiry date</label>
                    <input id="expiry_date" name="expiry_date" type="date" value="<?php echo e(old('expiry_date')); ?>" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-[13px] dark:border-gray-700 dark:bg-gray-900">
                </div>
                <div>
                    <label class="mb-1 block text-[12px] font-medium text-gray-700 dark:text-gray-300">Batch code</label>
                    <input id="batch_code" name="batch_code" type="text" value="<?php echo e(old('batch_code')); ?>" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-[13px] dark:border-gray-700 dark:bg-gray-900">
                </div>
            </div>

            <div>
                <label class="mb-1 block text-[12px] font-medium text-gray-700 dark:text-gray-300">Notes</label>
                <textarea name="notes" rows="3" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-[13px] dark:border-gray-700 dark:bg-gray-900"><?php echo e(old('notes')); ?></textarea>
            </div>
        </section>
        ...
        <div class="flex justify-end gap-2">
            <a href="<?php echo e(route('admin.inventory.packs.index')); ?>" class="rounded border border-gray-300 px-4 py-2 text-xs hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800">Cancel</a>
    ...
    <?php if($recentPacks->isNotEmpty()): ?>
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-950">
            ...
            <div class="mt-3 grid gap-2 md:grid-cols-2">
                <?php $__currentLoopData = $recentPacks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pack): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="rounded-lg border border-gray-200 p-3 text-[11px] dark:border-gray-800">
                        ...
                        <div class="text-gray-500 dark:text-gray-400"><?php echo e($pack->pack_code ?? 'Pack #' . $pack->id); ?> · <?php echo e($pack->status); ?></div>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
        </div>
...
(function () {
    ...
    const lotPiecesMeta = <?php echo json_encode($lotPiecesMeta, 15, 512) ?>;
    const initialOutputs = <?php echo json_encode($initialOutputs, 15, 512) ?>;
    const oldSourcePiecesPerUnit = <?php echo json_encode(old('source_pieces_per_unit'), 15, 512) ?>;
...
<?php echo $__env->make('layouts.company', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/ooglie/Website/ChatGPT/PRODUCTIONFrozen/BandaraFrozen/resources/views/admin/inventory/packs/create.blade.php ENDPATH**/ ?>