...
(function () {
    ...
    const outputOptionsByLot = @json($outputOptionsByLot ?? []);
    ...
    function selectedOutputContext() {
        ...
        const context = outputOptionsByLot[String(source.value)] || null;
    ...
    function populateOutputProducts(row, desiredProductId = null) {
        ...
        placeholder.value = '';
        placeholder.textContent = source?.value
            ? (products.length > 0 ? 'Select associated output product…' : 'No associated output product configured')
        ...
        select.disabled = !source?.value || products.length === 0;

        const previousIsAllowed = products.some(product => String(product.id) === previous);
        if (previousIsAllowed) {
            select.value = previous;
        } else if (products.length === 1) {
            // One exact associated product: select it automatically and let the
            // user choose only the required 10pc/20pc/etc. output variant.
    ...
    function updateTransformationHint() {
        ...
        if (!source?.value) {
            ...
            transformationHint.textContent = `${products.length} products are associated with this source. No products from other source relationships are loaded.`;
        } else if (products.length === 1) {
