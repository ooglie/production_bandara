...
class InventoryPackController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $sourceProductIds = $lots->pluck('product_id')
            ->filter()
            ->map(fn ($id) => (int) $id)
            ->unique()
            ->values();
        ...
        $transformationTargetsBySource = ProductTransformation::query()
            ...
            ->when($sourceProductIds->isNotEmpty(), fn ($query) => $query->whereIn('source_product_id', $sourceProductIds->all()))
            ->when($sourceProductIds->isEmpty(), fn ($query) => $query->whereRaw('1 = 0'))
            ->get()
            ->groupBy('source_product_id')
            ->map(fn ($rows) => $rows->pluck('target_product_id')
                ->map(fn ($id) => (int) $id)
                ->unique()
                ->values());

        // Build an exact output catalogue for each source lot. The browser never
        // receives a flat/global product list, so changing the source lot cannot
        // expose products associated with a different source product.
        $eligibleOutputProductIds = $sourceProductIds
            ->merge($transformationTargetsBySource->flatten())
            ->map(fn ($id) => (int) $id)
            ->unique()
            ->values();
        ...
        $outputProductsById = Product::query()
            ->select($productColumns)
            ->when($eligibleOutputProductIds->isNotEmpty(), fn ($query) => $query->whereIn('id', $eligibleOutputProductIds->all()))
            ->when($eligibleOutputProductIds->isEmpty(), fn ($query) => $query->whereRaw('1 = 0'))
            ->get()
            ->keyBy('id');
        ...
        $outputVariantsByProduct = ProductVariant::query()
            ->select($variantColumns)
            ->when($eligibleOutputProductIds->isNotEmpty(), fn ($query) => $query->whereIn('product_id', $eligibleOutputProductIds->all()))
            ->when($eligibleOutputProductIds->isEmpty(), fn ($query) => $query->whereRaw('1 = 0'))
            ->orderBy('name')
            ->orderBy('id')
            ->get()
        ...
        $outputOptionsByLot = $lots->mapWithKeys(function (InventoryLot $lot) use ($transformationTargetsBySource, $outputProductsById, $outputVariantsByProduct) {
            $sourceProductId = (int) $lot->product_id;
            $allowedProductIds = collect($transformationTargetsBySource->get($sourceProductId, collect()))
                // Same-product variant repacking, such as 240pc Dimsum -> 10pc/20pc,
                // remains available alongside explicitly mapped target products.
            ...
            $products = $allowedProductIds
                ->map(function (int $productId) use ($outputProductsById, $outputVariantsByProduct) {
                    /** @var Product|null $product */
                    $product = $outputProductsById->get($productId);
                    if (! $product) {
                        return null;
                    }

                    $variants = collect($outputVariantsByProduct->get($productId, collect()))
        ...
        return view('admin.inventory.packs.create', compact('lots', 'outputOptionsByLot', 'recentPacks', 'selectedLotId'));
    }
