...
class CartService
{
    ...
    public function syncPrices(Cart $cart): int
    {
        ...
        foreach ($items as $it) {
            if (in_array((string) ($it->b2b_order_mode ?? ''), ['pieces', 'weight'], true)) {
                // B2B variable-weight lines are intentionally zero-value until
                // staff enters actual supplied weight and finalizes the invoice.
                continue;
            }
    ...
    private function resolveUnitPrice(Product $product, ?ProductVariant $variant): float
    {
        ...
        if (class_exists(\App\Services\PricingService::class)) {
            $resolved = (float) app(\App\Services\PricingService::class)->priceFor($user, $product, $variant);
     ...
     * - product is configured as "price includes GST"
    ...
    private function normalizeVariantUnitPriceForCart(Product $product, ?ProductVariant $variant, float $resolvedPrice): float
    {
        ...
        if (!($product->price_includes_gst ?? false) || $gstRate <= 0) {
            return round($resolvedPrice, 2);
        }
