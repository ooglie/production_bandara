...
class CartController extends Controller
{
    ...
    public function add(Request $request, CartService $cartService)
    {
        $data = $request->validate([
            ...
            'b2b_order_mode'   => ['nullable', 'in:pieces,weight'],
            'requested_piece_count' => ['nullable', 'integer', 'min:1'],
            'requested_weight_kg' => ['nullable', 'numeric', 'min:0.001'],
        ]);
        ...
        try {
            ...
            if ($isB2B && (! empty($data['inventory_piece_id']) || ! empty($data['piece_weight_kg']))) {
                throw ValidationException::withMessages([
                    'product_id' => 'B2B customers order from the controlled B2B catalogue. Variant, slab, and stock choices are allocated by the team.',
                ]);
            }

            if ($isB2B && $this->productNeedsB2BWeightFinalization($product, $requestedSellUnit) && ! $variantId) {
                return $this->addB2BPendingWeightCartItem($request, $cartService, $product, $requestedSellUnit, $data);
            }

            if ($isB2B && $this->productHasVariants($product) && ! $variantId) {
                throw ValidationException::withMessages([
                    'product_id' => 'Please choose an assigned B2B buying option before adding this product to cart.',
                ]);
            }
            ...
            if ($isB2B && ! app(\App\Services\B2BTermsService::class)->canBuy($user, $product, $requestedSellUnit ?: $variant?->sellUnit, $variant)) {
                throw ValidationException::withMessages([
                    'product_id' => 'This product option is not assigned to your B2B portfolio. Please request access before buying.',
                ]);
            }
    ...
    {
        return ($request->user()
            && (($request->user()->customer_type ?? 'b2c') === 'b2b')
            && \Illuminate\Support\Facades\Route::has('b2b.cart.index'))
                ? 'b2b.cart.index'
                : 'cart.index';
    }
    ...
    private function createSelectedPieceCartItem(int $cartId, Product $product, ?int $variantId, InventoryPiece $selectedPiece, ?float $overrideUnitPrice = null): void
    {
        ...
        $unitPrice = $overrideUnitPrice !== null
            ...
            : (float) ($product->effective_price ?? 0);
    ...
    private function reapplySelectedPiecePricing($cart): int
    {
        ...
        foreach ($items as $item) {
            ...
            $unitPrice = (float) ($item->product?->effective_price ?? $item->unit_price ?? 0);
            $newTotal = $isKg
                ? round($unitPrice * $weightKg, 2)
                : round($unitPrice, 2);
    ...
    private function productNeedsB2BWeightFinalization(Product $product, ?ProductSellUnit $sellUnit = null): bool
    {
        $unitType = strtolower((string) ($sellUnit?->unit_type ?? ''));
        if (in_array($unitType, ['kg', 'request_piece', 'request_weight'], true)) {
            return true;
        }

        if (strtolower((string) ($product->sell_unit ?? 'piece')) === 'kg') {
            return true;
        }

        // Variable products without a fixed, linked sellable-unit/variant need
        // actual weight/stock confirmation by staff before invoicing.
        return $this->productHasVariants($product) && ! $sellUnit;
    }

    private function addB2BPendingWeightCartItem(Request $request, CartService $cartService, Product $product, ?ProductSellUnit $sellUnit, array $data)
    {
        $mode = (string) ($data['b2b_order_mode'] ?? '');
        if (! in_array($mode, ['pieces', 'weight'], true)) {
            throw ValidationException::withMessages([
                'b2b_order_mode' => 'Please choose whether to order by pieces or approximate kg.',
            ]);
        }
        ...
        if ($mode === 'pieces') {
            $requestedPieces = max(1, (int) ($data['requested_piece_count'] ?? 0));
            if ($requestedPieces <= 0) {
                throw ValidationException::withMessages([
                    'requested_piece_count' => 'Please enter the number of pieces required.',
                ]);
            }
        } else {
            $requestedWeight = round((float) ($data['requested_weight_kg'] ?? 0), 3);
            if ($requestedWeight <= 0) {
                throw ValidationException::withMessages([
                    'requested_weight_kg' => 'Please enter the approximate kg required.',
                ]);
            }
        }
        ...
        $unitPrice = (float) ($sellUnit
            ? app(\App\Services\PricingService::class)->priceForSellUnit($user, $product, $sellUnit)
            : app(\App\Services\PricingService::class)->priceFor($user, $product));

        if ($unitPrice <= 0) {
            throw ValidationException::withMessages([
                'product_id' => 'B2B pricing is pending for this product option.',
            ]);
        }

        $cart = $cartService->currentCart(true);
        if (! $cart) {
            throw ValidationException::withMessages(['cart' => 'Unable to create cart.']);
        }
        ...
        if ($this->hasCartItemColumn('product_sell_unit_id') && $sellUnit) {
            $item->product_sell_unit_id = $sellUnit->id;
        }
        if ($this->hasCartItemColumn('b2b_order_mode')) {
            $item->b2b_order_mode = $mode;
        }
        if ($this->hasCartItemColumn('requested_piece_count')) {
            $item->requested_piece_count = $requestedPieces;
        }
        if ($this->hasCartItemColumn('requested_weight_kg')) {
            $item->requested_weight_kg = $requestedWeight;
        }

        $item->quantity = $mode === 'pieces' ? (float) $requestedPieces : (float) $requestedWeight;
        $item->unit_price = $unitPrice;
        $item->item_weight = $requestedWeight;
        // Final amount is unknown until staff enters actual shipped weight.
        $item->total = 0;
        $item->save();

        $cart->touch();

        return back()->with('status', 'B2B order item added. Final weight and invoice amount will be confirmed by the team.');
    }
