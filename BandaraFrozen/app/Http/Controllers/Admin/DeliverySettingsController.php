...
class DeliverySettingsController extends Controller
{
    ...
    private function normalizeDistanceRule(Request $request, array $data): array
    {
        ...
        $data['included_distance_km'] = $request->filled('included_distance_km')
            ? round((float) $data['included_distance_km'], 2)
            : 0.0;
        $data['per_km_fee'] = $request->filled('per_km_fee') ? round((float) $data['per_km_fee'], 2) : null;
