...
(function () {
    ...
    function populateOutputProducts(row, desiredProductId = null) {
        ...
        const sourceProductId = selectedSourceProductId();
        const allowedIds = mappedTargetIdsForSelectedSource();
        const previous = desiredProductId !== null ? String(desiredProductId || '') : String(select.value || '');

        select.innerHTML = '';

        const placeholder = document.createElement('option');
        placeholder.value = '';
        placeholder.textContent = sourceProductId
            ? (allowedIds.length > 0 ? 'Select associated output product…' : 'No associated output product configured')
            : 'Select a source lot first…';
        select.appendChild(placeholder);

        allowedIds.forEach(productId => {
            const product = productMeta[String(productId)];
            if (!product) return;
        ...
        select.disabled = !sourceProductId || allowedIds.length === 0;
        select.value = allowedIds.includes(previous) ? previous : '';
    }
    ...
    function refreshAllRows() {
        rowElements().forEach(row => {
            const previousProduct = row.querySelector('[data-output-product]')?.value || '';
    ...
    function updateTransformationHint() {
        ...
        if (!sourceProductId) {
            transformationHint.textContent = 'Select a source lot. Only products associated with that source will be available as outputs.';
        } else if (mappedIds.length > 1) {
            transformationHint.textContent = `${mappedIds.length} associated output products are available for this source, including its own pack variants.`;
        } else if (mappedIds.length === 1) {
            transformationHint.textContent = 'Only this source product and its configured pack variants are available as outputs.';
        } else {
