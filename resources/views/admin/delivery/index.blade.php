...
<div class="max-w-7xl mx-auto px-4 py-6 space-y-6">
    ...
    <section class="rounded-2xl border border-blue-100 bg-blue-50/80 p-5 dark:border-blue-900/50 dark:bg-blue-950/30">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            ...
            <a href="{{ route('admin.hsn-codes.index') }}" class="inline-flex items-center justify-center rounded-xl border border-blue-200 bg-white px-3 py-2 text-xs font-medium text-blue-800 hover:bg-blue-50 dark:border-blue-800 dark:bg-blue-950 dark:text-blue-100">
                Manage HSN codes
            </a>
        </div>

        <form method="POST" action="{{ route('admin.delivery.tax-settings.update') }}" class="mt-4 space-y-4">
            @csrf
            @method('PUT')
            <div class="grid gap-4 md:grid-cols-2">
                <label class="space-y-1 text-[11px] font-medium text-blue-950 dark:text-blue-50">
                    ...
                    <select name="delivery_hsn_code_id" class="w-full rounded-xl border border-blue-200 bg-white px-3 py-2 text-xs text-gray-900 dark:border-blue-800 dark:bg-gray-950 dark:text-gray-100">
                        ...
                        @foreach($hsnCodes as $hsn)
                            <option value="{{ $hsn->id }}" @selected($currentDeliveryHsnId === (int) $hsn->id)>{{ $formatHsnOption($hsn) }}</option>
                        @endforeach
                    </select>
                    <span class="block text-[10px] font-normal text-blue-900/70 dark:text-blue-100/70">
                        Current delivery invoice code: <strong>{{ $currentDeliverySac }}</strong> · GST <strong>{{ $formatPercent($currentDeliveryGstRate) }}</strong>
                    </span>
                </label>
                ...
                <label class="space-y-1 text-[11px] font-medium text-blue-950 dark:text-blue-50">
                    ...
                    <select name="handling_hsn_code_id" class="w-full rounded-xl border border-blue-200 bg-white px-3 py-2 text-xs text-gray-900 dark:border-blue-800 dark:bg-gray-950 dark:text-gray-100">
                        ...
                        @foreach($hsnCodes as $hsn)
                            <option value="{{ $hsn->id }}" @selected($currentHandlingHsnId === (int) $hsn->id)>{{ $formatHsnOption($hsn) }}</option>
                        @endforeach
                    </select>
                    <span class="block text-[10px] font-normal text-blue-900/70 dark:text-blue-100/70">
                        Current cold-chain invoice code: <strong>{{ $currentHandlingSac }}</strong> · GST <strong>{{ $formatPercent($currentHandlingGstRate) }}</strong>
                    </span>
                </label>
            </div>
    ...
    <section class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @foreach($distanceRules as $rule)
                        ...
                        <tr>
                            ...
                            <td class="whitespace-nowrap"><input form="{{ $distanceRuleFormId }}" name="tax_rate" type="number" step="0.01" min="0" value="{{ $hasDeliveryServiceHsn ? $currentDeliveryGstRate : $rule->tax_rate }}" readonly class="w-20 rounded-lg border border-gray-300 bg-gray-100 px-2 py-1 text-gray-600 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300"><div class="mt-1 text-[10px] text-gray-400">{{ $hasDeliveryServiceHsn ? $currentDeliverySac : 'Manual / legacy' }}</div></td>
        ...
        <form method="POST" action="{{ route('admin.delivery.distance-rules.store') }}" class="mt-4 rounded-2xl border border-gray-100 bg-gray-50 p-4 dark:border-gray-800 dark:bg-gray-950/40">
            ...
            <div class="grid gap-3 md:grid-cols-3 xl:grid-cols-5">
                ...
                <label class="space-y-1 text-[11px] font-medium text-gray-600 dark:text-gray-300">
                    <span>GST % auto</span>
                    <input type="hidden" name="tax_rate" value="{{ $currentDeliveryGstRate }}">
                    <div class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-xs text-gray-600 dark:border-gray-800 dark:bg-gray-950/40 dark:text-gray-300">
                        GST {{ $formatPercent($currentDeliveryGstRate) }}<br><span class="text-[10px] text-gray-400">{{ $currentDeliverySac }}</span>
                    </div>
                </label>
    ...
    <section class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse($handlingRules as $rule)
                        <tr>
                            ...
                            <td class="whitespace-nowrap"><input name="tax_rate" form="handling-rule-update-{{ $rule->id }}" type="number" step="0.01" min="0" value="{{ $hasHandlingServiceHsn ? $currentHandlingGstRate : $rule->tax_rate }}" readonly class="w-20 rounded-lg border border-gray-300 bg-gray-100 px-2 py-1 text-gray-600 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300"><div class="mt-1 text-[10px] text-gray-400">{{ $hasHandlingServiceHsn ? $currentHandlingSac : 'Manual / legacy' }}</div></td>
        ...
        <form method="POST" action="{{ route('admin.delivery.handling-rules.store') }}" class="mt-4 grid gap-2 md:grid-cols-8">
            ...
            <input type="hidden" name="tax_rate" value="{{ $currentHandlingGstRate }}">
            <div class="rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-xs text-gray-600 dark:border-gray-800 dark:bg-gray-950/40 dark:text-gray-300">
                GST {{ $formatPercent($currentHandlingGstRate) }}<br><span class="text-[10px] text-gray-400">{{ $currentHandlingSac }}</span>
            </div>
