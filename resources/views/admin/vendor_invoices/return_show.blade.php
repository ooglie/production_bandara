    ...
    <section class="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-hidden">
        ...
        <div class="overflow-x-auto"><table class="min-w-full text-[11px]"><thead class="bg-gray-50 dark:bg-gray-950/40"><tr><th class="px-4 py-3 text-left">Product / lot</th><th class="px-4 py-3 text-right">Return</th><th class="px-4 py-3 text-right">Taxable</th><th class="px-4 py-3 text-right">Tax</th><th class="px-4 py-3 text-right">Total</th></tr></thead><tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            @foreach($vendorReturn->items as $item)
                <tr><td class="px-4 py-3"><div class="font-medium">{{ $item->invoiceItem?->product?->name ?? '—' }}</div>@if($item->invoiceItem?->productVariant)<div class="text-[10px] text-gray-400">{{ $item->invoiceItem->productVariant->name }}</div>@endif<div class="text-[10px] text-gray-400">{{ $item->inventoryLot?->lot_code ?? 'Lot unavailable' }} · {{ ucwords(str_replace('_', ' ', $item->return_mode)) }}</div></td><td class="px-4 py-3 text-right">@if((float)$item->weight_kg > 0){{ number_format((float)$item->weight_kg, 3) }} kg @else {{ number_format((float)$item->quantity, 3) }} units @endif @if((int)$item->piece_count > 0)<div class="text-[10px] text-gray-400">{{ $item->piece_count }} pcs</div>@endif</td><td class="px-4 py-3 text-right">₹{{ number_format((float)$item->subtotal_amount, 2) }}</td><td class="px-4 py-3 text-right">₹{{ number_format((float)$item->tax_amount, 2) }}</td><td class="px-4 py-3 text-right font-semibold">₹{{ number_format((float)$item->total_amount, 2) }}</td></tr>
            @endforeach
        </tbody></table></div>
    </section>
