...
@section('title', 'Correct Vendor Invoice Details')
@section('breadcrumb', 'Admin · Vendor Invoices · Correct details')
...
<div class="max-w-4xl mx-auto px-4 py-6 space-y-4 text-xs">
    <div class="flex items-start justify-between gap-3">
        <div>
            ...
            <h1 class="mt-1 text-lg font-semibold text-gray-900 dark:text-gray-50">Correct {{ $invoice->invoice_number }}</h1>
    ...
    <form method="POST" action="{{ route('admin.vendor-invoices.update-details', $invoice) }}" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <section class="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-5 space-y-4">
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input name="invoice_number" value="{{ old('invoice_number', $invoice->invoice_number) }}" required maxlength="100"
                ...
                <div>
                    ...
                    <input name="tally_reference" value="{{ old('tally_reference', $invoice->tally_reference) }}" maxlength="255"
                           class="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-950 px-3 py-2 text-[12px]">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="notes" rows="4" maxlength="10000"
                          class="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-950 px-3 py-2 text-[12px]">{{ old('notes', $invoice->notes) }}</textarea>
            </div>
        </section>
        ...
        <section class="rounded-2xl border border-amber-200 dark:border-amber-900 bg-amber-50/70 dark:bg-amber-950/20 p-5">
            ...
            <textarea name="correction_reason" rows="3" required maxlength="500"
                      ...
                      class="w-full rounded-lg border border-amber-300 dark:border-amber-800 bg-white dark:bg-gray-950 px-3 py-2 text-[12px]">{{ old('correction_reason') }}</textarea>
        ...
        <div class="flex items-center justify-end gap-2">
            <a href="{{ route('admin.vendor-invoices.show', $invoice) }}" class="px-3 py-2 text-[11px] text-gray-500 hover:underline">Cancel</a>
