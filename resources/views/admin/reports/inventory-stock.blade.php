...
@section('breadcrumb', 'Admin · Reports · Inventory Stock')
...
<div class="max-w-7xl mx-auto px-4 py-5 text-xs space-y-5">
    ...
    <form method="GET" action="{{ route('admin.reports.inventory-stock') }}" class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-950">
        <div class="grid gap-3 md:grid-cols-8 items-end">
            <div>
                ...
                <select name="stock_type" class="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-[12px] dark:border-gray-700 dark:bg-gray-900">
                    ...
                    @foreach($stockTypeOptions as $value => $label)
                        <option value="{{ $value }}" @selected(($filters['stock_type'] ?? '') === $value)>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div>
                ...
                <select name="expiry" class="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-[12px] dark:border-gray-700 dark:bg-gray-900">
                    ...
                    <option value="next_7" @selected(($filters['expiry'] ?? '') === 'next_7')>Next 7 days</option>
                    <option value="next_15" @selected(($filters['expiry'] ?? '') === 'next_15')>Next 15 days</option>
                    <option value="next_30" @selected(($filters['expiry'] ?? '') === 'next_30')>Next 30 days</option>
                    <option value="next_60" @selected(($filters['expiry'] ?? '') === 'next_60')>Next 60 days</option>
                </select>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="number" name="limit" min="1" max="{{ $maxLimit }}" value="{{ $limit }}" class="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-[12px] dark:border-gray-700 dark:bg-gray-900">
            </div>
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.reports.inventory-stock.export', request()->query()) }}" class="rounded-lg border border-gray-300 px-3 py-2 text-[12px] hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-900">
                    CSV
                </a>
            </div>
        </div>
    ...
    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        @foreach([
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-950">
                ...
                <div class="mt-1 text-xl font-semibold text-gray-900 dark:text-gray-50">{{ number_format((int) ($summary[$key] ?? 0)) }}</div>
            </div>
        @endforeach
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-3 text-[12px] text-gray-500 dark:border-gray-800 dark:bg-gray-950 dark:text-gray-400">
        Showing up to {{ number_format($limit) }} rows. Use CSV export for the same filtered result set up to {{ number_format($maxLimit) }} rows.
    </div>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                @forelse($rows as $row)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/40">
                        <td class="px-4 py-3">
                            <span class="inline-flex rounded-full border px-2 py-0.5 text-[11px] {{ $labelClass($row['stock_type']) }}">
                                {{ $row['type_label'] }}
                            </span>
                        </td>
