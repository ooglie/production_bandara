...
@section('title', 'Products Without Stock')
@section('breadcrumb', 'Admin · Reports · Products Without Stock')

@section('content')
<div class="max-w-5xl mx-auto px-4 py-5 text-xs space-y-5">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-lg font-semibold text-gray-900 dark:text-gray-50">Products Without Stock</h1>
            <p class="text-[12px] text-gray-500 dark:text-gray-400">
                Active inventory-managed products whose available catalog stock is zero.
            </p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('admin.reports.products-without-stock.export') }}" class="inline-flex items-center rounded-xl border border-gray-900 bg-gray-900 px-3 py-2 text-[12px] font-semibold text-white hover:bg-gray-800 dark:border-gray-100 dark:bg-gray-100 dark:text-gray-900 dark:hover:bg-gray-200">
                Export CSV
            </a>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-3 text-[12px] text-gray-500 dark:border-gray-800 dark:bg-gray-950 dark:text-gray-400">
        {{ number_format($rows->total()) }} {{ \Illuminate\Support\Str::plural('product', $rows->total()) }} without stock.
    ...
    <div class="overflow-x-auto rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950">
        <table class="min-w-full text-[12px]">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                @empty
                    <tr>
                        <td colspan="3" class="px-4 py-8 text-center text-gray-500">All active inventory-managed products currently have stock.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
