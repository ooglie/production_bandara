    ...
    <main>
        ...
        <section class="mx-auto w-full max-w-7xl px-4 pb-16 sm:px-6 sm:pb-20 lg:px-8" aria-label="Published Chefs">
            @if ($chefs->isNotEmpty())
                ...
                @if ($chefs->hasPages())
                    <div class="mt-10">{{ $chefs->links() }}</div>
                @endif
            @else
                <div class="rounded-xl border border-slate-200/80 bg-white px-6 py-12 text-center dark:border-slate-800 dark:bg-slate-950">
                    ...
                    <a href="{{ route('kitchen.index') }}" class="mt-6 inline-flex min-h-11 items-center justify-center rounded-lg border border-slate-300 px-5 py-2.5 text-sm font-medium text-slate-900 dark:border-slate-700 dark:text-slate-100">Return to Bandara Kitchen</a>
                </div>
            @endif
        </section>
    </main>
