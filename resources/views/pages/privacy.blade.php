    ...
    <div class="min-h-screen bg-stone-50 text-slate-800 dark:bg-slate-950 dark:text-slate-200">
        <header class="border-b border-stone-200 bg-white dark:border-slate-800 dark:bg-slate-950">
            <div class="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-20 lg:px-8">
                ...
                <p class="mt-5 max-w-3xl text-sm font-light leading-7 text-slate-600 dark:text-slate-300 sm:text-base">This policy explains how {{ $company['legal_name'] }} handles personal data when you browse, register, place an order, use B2B terms, contact support or interact with delivery services.</p>
        ...
        <div class="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[0.28fr_0.72fr] lg:px-8 lg:py-16">
            ...
            <article class="space-y-12 text-sm font-light leading-7 text-slate-600 dark:text-slate-300 [&_h2]:text-2xl [&_h2]:font-light [&_h2]:tracking-tight [&_h2]:text-slate-950 dark:[&_h2]:text-white [&_h3]:mt-6 [&_h3]:font-normal [&_h3]:text-slate-900 dark:[&_h3]:text-slate-100 [&_li]:ml-5 [&_li]:list-disc [&_p+p]:mt-4 [&_ul]:mt-4 [&_ul]:space-y-2">
                <section id="who-we-are" class="scroll-mt-28">
                    <h2>1. Who we are</h2>
                    <p>{{ $company['legal_name'] }} operates the Bandara shopping service at {{ $company['shopping_domain'] }}. For privacy questions, email <a href="mailto:{{ $company['privacy_email'] }}" class="underline underline-offset-4">{{ $company['privacy_email'] }}</a>. The registered address is {{ $company['registered_address'] }}.</p>
                </section>
                ...
                <section id="choices" class="scroll-mt-28">
                    ...
                    <p>Privacy enquiries may be sent to <a href="mailto:{{ $company['privacy_email'] }}" class="underline underline-offset-4">{{ $company['privacy_email'] }}</a>. General order support remains available at <a href="mailto:{{ $company['support_email'] }}" class="underline underline-offset-4">{{ $company['support_email'] }}</a>.</p>
                </section>
                ...
                <section id="changes-contact" class="scroll-mt-28">
                    ...
                    <div class="mt-6 rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900/70">
                        <p><strong class="font-normal text-slate-900 dark:text-slate-100">Privacy contact</strong><br><a href="mailto:{{ $company['privacy_email'] }}" class="underline underline-offset-4">{{ $company['privacy_email'] }}</a><br>{{ $company['registered_address'] }}</p>
                        <p class="mt-4"><strong class="font-normal text-slate-900 dark:text-slate-100">Grievance Officer</strong><br>{{ $company['grievance']['name'] }}, {{ $company['grievance']['designation'] }}<br><a href="mailto:{{ $company['grievance']['email'] }}" class="underline underline-offset-4">{{ $company['grievance']['email'] }}</a></p>
                    </div>
                    <p class="mt-5">For order and service questions, see <a href="{{ route('content.help') }}" class="underline underline-offset-4">Help & FAQs</a>. Contractual terms are in <a href="{{ route('content.terms') }}" class="underline underline-offset-4">Terms & Policies</a>.</p>
                </section>
            </article>
        </div>
    </div>
