...
return new class extends Migration
{
    public function up(): void
    {
        ...
        foreach (['base_price', 'mrp_price', 'weight_per_unit_kg'] as $column) {
        ...
        foreach ($sellUnitsByProduct as $productId => $sellUnits) {
            // Only auto-copy when there is one clear format. Multiple sellable
            // formats need deliberate product-level pricing on the product page.
            ...
            if (Schema::hasColumn('products', 'sell_unit')) {
                ...
                $saleType = (string) ($sellUnit->sale_type ?? '');
                $suggestedSellUnit = $saleType === 'variable_weight' ? 'kg' : 'pack';
    ...
    {
        // No-op. This only fills missing simple-product display defaults from
        // the product's one sellable format so the unchanged storefront has a price.
    }
};
